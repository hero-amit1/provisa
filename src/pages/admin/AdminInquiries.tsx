...
const AdminInquiries = () => {
  ...
  // Contact inquiries removed from admin panel; only appointments are shown.
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const data = await inquiriesAPI.getAppointments();
  ...
  const getStatusBadge = (status: string) => {
    ...
    return <Badge variant={variants[status] || 'secondary'}>{labels[status] || status || 'New'}</Badge>;
  };
  ...
  const renderAppointmentRow = (inquiry: Inquiry) => (
    <tr key={inquiry._id} className="border-b border-border hover:bg-muted/50">
      ...
      <td className="p-4">{inquiry.date ? new Date(inquiry.date).toLocaleDateString() : '-'}</td>
  ...
  return (
    <AdminLayout>
      <div className="flex items-center justify-between mb-6">
        <h2 className="font-heading font-semibold text-lg text-foreground">Manage Appointments</h2>
        <div className="flex gap-2">
          <button
            onClick={() => { }}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition flex items-center gap-2 bg-primary text-primary-foreground`}
          >
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border">
              ...
              <th className="text-left p-4 font-medium text-muted-foreground">Appointment Date</th>
              <th className="text-left p-4 font-medium text-muted-foreground">Submitted</th>
              <th className="text-center p-4 font-medium text-muted-foreground">Status</th>
              <th className="text-right p-4 font-medium text-muted-foreground">Actions</th>
            </tr>
          </thead>
          <tbody>{items.map(renderAppointmentRow)}</tbody>
        </table>
        {items.length === 0 && (
          <div className="p-8 text-center text-muted-foreground">No appointment bookings yet.</div>
        )}
      </div>
    </AdminLayout>
  );
